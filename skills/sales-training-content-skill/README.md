...
A Claude skill for building professional B2B **sales, customer success, and support** training programmes. It draws down your own real account and campaign data from the DemandIntel platform, applies established, tried and tested methodologies, and turns them into trainer notes, exercises, handouts, and assessments — grounded in your team's actual competency scores, objections, and calls rather than generic textbook scenarios.
...
**Every role, call type, and framework the DemandIntel platform serves:**
...
- Live grounding in your own DemandIntel account and campaign data where the MCP server is connected

## Grounding in your own data (recommended)

This skill is at its best when connected to the **DemandIntel MCP server** — it will pull your team's real competency scores, QA heatmaps, top objections, coaching plans, league tables, and call transcripts, and build every example, role-play, and assessment from them.

1. Mint a Personal Access Token at https://demandintel.io/app/settings → **Headless Access**.
2. Add the DemandIntel MCP server (https://mcp.demandintel.io) to your MCP client (Claude Code, Codex, Cursor, ChatGPT desktop, Continue, etc.).
...
2. Place the whole `sales-training-content-skill` folder (containing `SKILL.md` and the `references` folder) into your Claude skills directory.
3. In conversation, reference "DemandIntel training skill" or "DemandIntel sales training" alongside your request to invoke it, for example:

   > "Use the DemandIntel training skill to build a 90 minute objection handling session for my SDR team, grounded in our Northwind campaign data."
...
sales-training-content-skill/
  ...
  references/
    ...
    mcp-data-sourcing.md      How to draw down account & campaign context via the DemandIntel MCP server
...
DemandIntel is a conversational intelligence and go to market consultancy. Find out more at demandintel.io.
